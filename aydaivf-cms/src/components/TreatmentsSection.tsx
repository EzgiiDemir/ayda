...
export default function TreatmentsSection({ data, locale }: { data: TreatmentsSectionDTO; locale: string }) {
    return (
        <section
            ...
            style={{
                backgroundImage: `linear-gradient(90deg, rgba(255,255,255,0.68), rgba(255,255,255,0.68)), url(${data.background})`,
        ...
        >
            <div className="container flex flex-col items-center">
                ...
                <div className="text-sm md:text-base text-ayda-gray-dark text-center max-w-3xl">
                    <p>{data.intro}</p>
                    <p className="mt-2">{data.intro2}</p>
                </div>

                <div className="flex gap-4 justify-center items-center flex-wrap max-w-[700px] mx-auto mt-6">
                    {data.treatments.map((t) => (
                        <a key={t.slug} href={`/${locale}/treatments/${t.slug}`} className="text-xs md:text-sm text-ayda-gray-dark font-medium flex gap-1 items-center">
                            <svg className="w-4 h-4 text-ayda-pink-dark" fill="currentColor" viewBox="0 0 448 512">
                            ...
                            <p className="text-center hover:text-ayda-pink-dark transition-colors duration-300 ease-in cursor-pointer">{t.label}</p>
                        </a>
                    ))}
                </div>
