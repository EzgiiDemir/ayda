// src/components/site/Footer.tsx
import type { FooterDTO } from "@/lib/cms";
import { MapPin, Phone, Link2, Facebook, Instagram, Youtube, Link as LinkIcon } from "lucide-react";

const API_BASE = (process.env.NEXT_PUBLIC_API ?? process.env.NEXT_PUBLIC_API_URL ?? "").replace(/\/+$/, "");
const toAbs = (u?: string | null) =>
    !u ? "" : /^https?:|^data:/i.test(u) ? u : `${API_BASE}${u.startsWith("/") ? u : `/${u}`}`;

function SocialIcon({ platform }: { platform?: string }) {
    const p = (platform || "").toLowerCase();
    if (p.includes("facebook")) return <Facebook className="w-5 h-5 text-white" />;
    if (p.includes("instagram")) return <Instagram className="w-5 h-5 text-white" />;
    if (p.includes("youtube")) return <Youtube className="w-5 h-5 text-white" />;
    return <LinkIcon className="w-5 h-5 text-white" />;
}

export default function Footer({ data }: { data: FooterDTO }) {
    const socials = Array.isArray(data.socials) ? data.socials : [];
    const quick = Array.isArray(data.quicklinks) ? data.quicklinks : [];

    return (
        <footer className="mt-auto bg-ayda-blue pt-10 flex flex-col gap-10">
            {/* KUTULAR: her zaman yan yana */}
            <div className="container grid grid-cols-3 gap-5">
                {/* Adres */}
                <div className="bg-white rounded-md p-5 flex flex-col justify-center items-center w-full">
                    <div className="relative w-8 h-8 md:w-10 md:h-10 rounded-md bg-ayda-pink-dark p-2 hover:bg-ayda-blue transition-colors duration-300 ease-in flex justify-center items-center my-2 md:my-4">
                        <MapPin className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    </div>
                    <div className="flex flex-col gap-2 md:gap-4 flex-1 items-center">
                        <p className="text-ayda-dark capitalize text-lg md:text-[22px] font-medium text-center">
                            {data.address_title}
                        </p>
                        <div className="flex flex-col gap-3 items-center">
                            {data.address_badge ? (
                                // eslint-disable-next-line @next/next/no-img-element
                                <img alt="address_badge" className="w-[65px] h-[65px] object-cover" src="https://api.aydaivf.com/uploads/iso1_659752db23.png"/>
                            ) : null}
                            <p className="text-center text-base text-ayda-gray-dark">{data.address_text}</p>
                        </div>
                    </div>
                </div>

                {/* İletişim */}
                <div className="bg-white rounded-md p-5 flex flex-col justify-center items-center w-full">
                    <div className="relative w-8 h-8 md:w-10 md:h-10 rounded-md bg-ayda-pink-dark p-2 hover:bg-ayda-blue transition-colors duration-300 ease-in flex justify-center items-center my-2 md:my-4">
                        <Phone className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    </div>
                    <div className="flex flex-col flex-1 gap-2 md:gap-4 items-center">
                        <p className="text-ayda-dark capitalize text-lg md:text-[22px] font-medium">
                            {data.contact_title || "İletişim"}
                        </p>
                        <div className="flex flex-col gap-2 items-center">
                            {data.phone ? (
                                <p className="text-base text-ayda-gray-dark capitalize">
                                    <a
                                        href={`https://wa.me/${(data.phone || "").replace(/\s+/g, "")}`}
                                        target="_blank"
                                        rel="noopener noreferrer"
                                        className="text-ayda-pink-light font-medium hover:text-ayda-pink-dark"
                                    >
                                        Tel : {data.phone}
                                    </a>
                                </p>
                            ) : null}

                            {data.email ? (
                                <p className="text-base text-ayda-gray-dark capitalize">
                                    <a
                                        href={`mailto:${data.email}`}
                                        className="text-ayda-pink-light lowercase font-medium hover:text-ayda-pink-dark"
                                    >
                                        E-mail : {data.email}
                                    </a>
                                </p>
                            ) : null}

                            {socials.length > 0 && (
                                <div className="flex items-center gap-2 justify-center">
                                    {socials.map((s, i) => (
                                        <a
                                            key={`${s.platform || "social"}-${s.url || i}`}
                                            href={s.url}
                                            target="_blank"
                                            rel="noopener noreferrer"
                                            className="w-10 h-10 cursor-pointer rounded-md bg-ayda-pink-dark p-2 hover:bg-ayda-blue transition-colors duration-300 ease-in flex justify-center items-center"
                                            aria-label={s.platform}
                                            title={s.platform}
                                        >
                                            <SocialIcon platform={s.platform} />
                                        </a>
                                    ))}
                                </div>
                            )}
                        </div>
                    </div>
                </div>

                {/* Hızlı Erişim */}
                <div className="bg-white rounded-md p-5 flex flex-col justify-center items-center w-full">
                    <div className="relative w-8 h-8 md:w-10 md:h-10 rounded-md bg-ayda-pink-dark p-2 hover:bg-ayda-blue transition-colors duration-300 ease-in flex justify-center items-center my-2 md:my-4">
                        <Link2 className="w-5 h-5 md:w-6 md:h-6 text-white" />
                    </div>
                    <div className="flex flex-col flex-1 gap-2 md:gap-4 items-center">
                        <p className="text-ayda-dark capitalize text-lg md:text-[22px] font-medium">Hızlı Erişim</p>
                        <div className="flex flex-col gap-2 items-center">
                            {quick.map((q, i) => (
                                <a
                                    key={`${q.href}-${q.label}-${i}`}
                                    className="text-base text-ayda-pink-light font-medium hover:text-ayda-pink-dark capitalize"
                                    href={q.href}
                                >
                                    {q.label}
                                </a>
                            ))}
                        </div>
                    </div>
                </div>
            </div>

            {/* Alt bar */}
            <div className="flex flex-row justify-center items-center gap-1 py-4 bg-[#706F6F4F] text-ayda-gray-light">
                <p className="text-sm">{data.copyright}</p>
                <span className="text-ayda-gray-light hidden md:inline-block">|</span>

                {data.footer_badge ? (
                    data.footer_badge_href ? (
                        <a href={data.footer_badge_href} target="_blank" rel="noopener noreferrer">
                            {/* eslint-disable-next-line @next/next/no-img-element */}
                            <img
                                src={toAbs(data.footer_badge)}
                                alt="footer-badge"
                                className="w-[180px] h-[18px] cursor-pointer object-contain"
                            />
                        </a>
                    ) : (
                        // eslint-disable-next-line @next/next/no-img-element
                        <img alt="footer-badge" className="w-[180px] h-[18px] cursor-pointer object-contain" src="data:image/png;base64," />
                    )
                ) : null}
            </div>
        </footer>
    );
}
